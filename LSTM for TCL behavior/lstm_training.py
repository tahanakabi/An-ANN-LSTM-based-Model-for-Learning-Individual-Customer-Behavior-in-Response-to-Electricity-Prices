...
nn_param_choices={'lstm_size':range(10,50,10),
                    'num_lstm':range(1,5),
...
def evolve(pop, retain=.25, random_select=.20, mutate_chance=0.15):
    ...
    graded.extend([list_to_save[i][-2:]+[pop[i]] for i in range(origins_length)])
    # Sort on the scores.
    sorted_nets = sorted(graded, key=lambda x: x[1])
    graded = [x[-1] for x in sorted_nets]

    #save for records
    list_to_save =  [list(element[-1].network.values())+element[:-1] for element in sorted_nets ]
